...
from widebrim.gamemodes.jiten.const import BIAS_HITBOX_SELECT_Y, COUNT_DISPLAY, NAME_ANIM_JITEN_BTN_SOLVE_OFF_WIFI, NAME_ANIM_JITEN_BTN_TOGGLE_FAV_OFF, NAME_ANIM_JITEN_BTN_TOGGLE_FAV_ON, NAME_ANIM_JITEN_TAG_ENCOUNTERED, NAME_ANIM_JITEN_TAG_HAT, NAME_ANIM_JITEN_TAG_SOLVED, NAME_ANIM_JITEN_TAG_UNTOUCHED, POS_CORNER_FAVOURITE, POS_CORNER_HITBOX_SELECT, SIZE_HITBOX_FAVOURITE, SIZE_HITBOX_SELECT
...
class JitenHitbox():
    def __init__(self, laytonState : Layton2GameState, idExternal : int, indexHitbox : int, imageAll : Optional[AnimatedImageObject], imageNum : Optional[AnimatedImageObject],
                 imageNumText : Optional[StaticImageAsNumericalFont], sourceGamemode):
        ...
        self.__textRendererName = StaticTextHelper(laytonState.fontEvent)
    ...
    def draw(self, gameDisplay):
        ...
        if self.__doDraw:
            ...
            if self.__imageNum != None:
                ...
                if (puzzleData := self.__laytonState.saveSlot.puzzleData.getPuzzleData(self.__idExternal - 1)) != None:

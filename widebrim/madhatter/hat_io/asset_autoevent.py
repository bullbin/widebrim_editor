...
class AutoEvent(File):
    ...
    def __str__(self):
        # Translated DebugPrintAutoEvent
        output = ""

        for indexPlace, entry in enumerate(self.entries):
            hasDefinitionStarted = False
            for indexSubPlace in range(AutoEventPlaceCollection.MAX_SUBROOM_COUNT):
                subPlace = entry.getSubPlaceEntry(indexSubPlace)
                if subPlace != None:
                    if not(hasDefinitionStarted):
                        output += "\n*** [AUTO EVENT] PLACE " + str(indexPlace) + " ***\n"
                        hasDefinitionStarted = True
                    output += "\n*** LT2_AUTO_EVENT::INFO %i: %i - %i ***\n" % (subPlace.idEvent, subPlace.chapterStart, subPlace.chapterEnd)
            
            if hasDefinitionStarted:
                output += "\n*** [AUTO EVENT] End PLACE %i ***\n\n" % indexPlace
